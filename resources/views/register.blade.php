...
<form class="tab-wizard2 wizard-circle wizard" action="{{ route('users.vitima.store') }}" method="POST" id="vitima-form">
    @csrf
    <h3>Credenciais Básicas da Conta</h3>
    ...
    $('#vitima-form').submit(function(event) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                ...
                var errors = xhr.responseJSON.message || 'Ocorreu um erro ao tentar criar a conta.';
                $('#response-message').html('<div class="alert alert-danger">' + errors + '</div>');
            }
        });
    });
});
